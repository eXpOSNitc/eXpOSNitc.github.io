...
\ENSURE 0 (Success) or -1 (File Descriptor given is invalid) or -2 (No disk space) or -3 (File is of type ROOT).
...
\begin{algorithmic}
\REQUIRE File Descriptor
\ENSURE 0 (Success) or -1 (File Descriptor is invalid) or -2 (File is of type ROOT). 
\STATE If file descriptor does not correspond to valid entry in Per Process Resource Table, return -1.
